...
def batch_trainer(epoch, model, train_loader, criterion,  optimizer, args, criterion1=None):
    ...
    for step, batch in enumerate(train_loader):
        ...
        train_loss = args.qratio * criterion(train_logits1, gt_label) + args.lratio * criterion(train_logits2, gt_label) #128*86
        # train_loss = criterion(train_logits1, gt_label)
        
        if criterion1 != None:
            train_loss = train_loss + 0.05 * criterion1(train_logits1, gt_label, args.gpu) + 0.1 * criterion1(train_logits2, gt_label, args.gpu)
...
def valid_trainer(model, valid_loader, criterion):
    ...
    with torch.no_grad():
        for step, batch in enumerate(valid_loader):
            ...
            preds_probs.append(valid_probs.cpu().detach().numpy())
