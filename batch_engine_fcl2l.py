...
def batch_trainer(epoch, model, train_loader, criterion,  optimizer, args, criterion1=None):
    model.train()
    ...
    for step, batch in enumerate(train_loader):
        ...
        # device = torch.device(args.device if torch.cuda.is_available() else "cpu")  # 指定GPU id，GPU id从0
        gt_label = batch['labels'].float().cuda()
        images = batch['image'].float().cuda()
        mask = batch['mask'].float().cuda()
        ...
        train_logits_l2l, train_logits_fc = model(images)
        train_logits1, train_logits2, mask, attns = train_logits_l2l
        # 需要替换的列的索引
        columns_to_replace = [2, 3, 7, 8, 9]  # Python索引是从0开始的，所以需要减1
        for col in columns_to_replace:
            train_logits1[:, col] = train_logits_fc[:, col]
            train_logits2[:, col] = train_logits_fc[:, col]
        train_loss = args.qratio * criterion(train_logits1, gt_label) + args.lratio * criterion(train_logits2, gt_label, mask) #128*86
...
def valid_trainer(model, valid_loader, criterion):
    ...
    with torch.no_grad():
        for step, batch in enumerate(valid_loader):
            ...
            valid_logits_l2l, valid_logits_fc = model(images)
            _, valid_logits, _, _ = valid_logits_l2l

            # 需要替换的列的索引
            columns_to_replace = [2, 3, 7, 8, 9]  # Python索引是从0开始的，所以需要减1
            for col in columns_to_replace:
                valid_logits[:, col] = valid_logits_fc[:, col]
            ...
            #     # print("valid gt[0]: ", valid_gt[0])
            #     print("valid probs: ", valid_probs[0])
